...
class MerkleTree {
  ...
  getPath(transaction) {
    ...
    // verify the path.
    path.result = this.traverseUp(path.result, i)
  ...
  traverseUp(arr, i){
    ...
    arr = this.traverseUp(arr, parent)
    return arr
  }
  isLeaf(i){
    return this.transactions.length < i * +1
  }
  ...
  verify(tx, path) {
    ...
    const hashed = []
    this.traverseAndHash(hashed, i)
    console.log(hashed)
  }

  traverseAndHash(hashed, i){
    if (i === 0) return;
    let par = (i-2)/2;
    hashed[par] = utils.hash("" + hashed[i-1] + "," + hashed[i]);

    // Test to see if we are the right subnode.  If so, we can hash
    // with the left subnode to continue one level up.
    if (par%2 === 0) {
      this.traverseAndHash(hashed, par);
    }
  }
